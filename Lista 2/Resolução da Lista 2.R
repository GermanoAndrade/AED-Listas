...
#Agora, vamos calcular a quantidade de observações (100α(\alpha)%, 
#com α(\alpha)=0,10) a qual vamos precisar remover essa quantidade 
#das menores observações e essa quantidade das maiores:
Qtd_Observ <- (length(Mono_Carb_O)*0.10)
Qtd_Observ

#Agora, removemos 12 das menores e das maiores observções, e calculamos a média 
...
#Agora, removemos 3 das menores e das maiores observções, e calculamos a média 
#(assim, teremos a média aparada):
...
ggplot(data = cd_salarios, aes(x = Mecânico)) + 
  ...
  labs(title = "Histograma Salário de Mecânicos", 
       x = "Salario dos mecânicos(em francos suiços)", 
       y = "Frequência") + 
  theme_classic()
...
ggplot(data = cd_salarios, aes(y = Mecânico)) + 
  ...
  labs(title = "Box Plot Salário de Mecânicos", 
       y = "Salario dos mecânicos(em francos suiços)") + 
  theme_classic()
...
#Então como i vai do primeiro elemento (x1) à mediana, vamos considerar i como de 1 até 
#metade das observações (i = n/2)

i <- 1:(length(Ubatuba)/2)
#Temos que  1≤($\leq$)i≤($\geq$)60
i

#Agora, vamos calcular os ui
ui <- median(Ubatuba)-Ubatuba[i]

#Para calcular os vi, vamos apenas reordernar o vetor em ordem decrescente e
...
GDP3 <- ggplot(data = cd_poluicao, aes(x = Umidade, y = co)) +
  ...
  scale_y_continuous(breaks = seq(0,14, 2), 
                     limits = c(0,14)) + 
  labs(title = "Gráfico de disersão 3: Umidade X Monóxido de Carbono (CO)", x = "Umidade", 
       y = "Monóxido de Carbono (CO)") + 
  theme_classic() +
  theme(panel.grid = element_line(linetype = "dashed", 
                                  colour = "white"), 
