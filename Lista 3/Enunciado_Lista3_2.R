...
## -----------------------------------------------------------------------------------------
Dados_Corret_A <-c(min(corretora.A), quantile(c(corretora.A$periodo_1, corretora.A$periodo_2, corretora.A$periodo_3), probs = c(0.25, 0.5, 0.75)), max(corretora.A))
Dados_Corret_A_M <- matrix(Dados_Corret_A, nrow = 1, ncol = 5, dimnames = list("A", c("x1", "q1", "q2", "q3", "xn")))
Dados_Corret_A_M
...
## -----------------------------------------------------------------------------------------
Dados_Corret_B <-c(min(corretora.B), quantile(c(corretora.B$periodo_1, corretora.B$periodo_2, corretora.B$periodo_3), probs = c(0.25, 0.5, 0.75)), max(corretora.B))
Dados_Corret_B_M <- matrix(Dados_Corret_B, nrow = 1, ncol = 5, dimnames = list("A", c("x1", "q1", "q2", "q3", "xn")))
...
#' 
## -----------------------------------------------------------------------------------------
C_A <- ggplot(data = data.frame(c(corretora.A$periodo_1, corretora.A$periodo_2, corretora.A$periodo_3)), aes(y = c(corretora.A$periodo_1, corretora.A$periodo_2, corretora.A$periodo_3))) + geom_boxplot(fill = "seagreen1", outlier.shape = 8, outlier.colour = "red", outlier.size = 2) + scale_x_continuous(b = NULL) + theme_classic() + labs(title = "Box Plot Corretora A", y = "% das ações") + scale_y_continuous(breaks = c(38, 54, 55, 60, 70))

## -----------------------------------------------------------------------------------------
C_B <- ggplot(data = data.frame(c(corretora.B$periodo_1, corretora.B$periodo_2, corretora.B$periodo_3)), aes(y = c(corretora.B$periodo_1, corretora.B$periodo_2, corretora.B$periodo_3))) + geom_boxplot(fill = "hotpink1") + scale_x_continuous(b = NULL) + theme_classic() + labs(title = "Box Plot Corretora B", y = "% das ações") + scale_y_continuous(breaks = c(50, 53, 56, 58, 61))
...
Corret_A <- c(corretora.A$periodo_1, corretora.A$periodo_2, corretora.A$periodo_3)
Corret_A
#Organizando os dados da corretora B em um único vetor:
Corret_B <- c(corretora.B$periodo_1, corretora.B$periodo_2, corretora.B$periodo_3)
Corret_B
n_A <- length(Corret_A)
n_B <- length(Corret_B)
